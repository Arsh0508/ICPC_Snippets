//not making a class, bcz no variables...
...
{
	using cmplx = complex<double>; // Can use long double for more precision but it will increase constant factor. It fuckin matters in FFT T_T
	...
	vector<T> doFFT(vector<T> &a, vector<T> &b) 
	{
		...
		for(S i = 0; i < n; i ++) 
		{
			result[i] = round(newA[i].real());//if T is floating point, don't round it.	
		}
		return result;
	}
};
